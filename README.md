# Projects-
Portfolio of Data Analytics projects showcasing Power BI dashboards, SQL analysis, Python basics, and business insights for data-driven decision making.
# 📊 Data Analytics Projects Portfolio

Welcome to my Data Analytics project repository.  
Here I showcase projects focused on data analysis, visualization, and business insights using tools like Power BI, SQL, Python, and Excel.

## 🔹 Key Skills Demonstrated
- Data Analysis & Visualization
- Dashboard Development (Power BI)
- SQL Data Querying & Reporting
- Business Insights & KPI Tracking
- Data Cleaning & Interpretation

## 🔹 Featured Projects
### Healthcare Revenue Dashboard
- Revenue trend analysis using Power BI
- Department-level insights & KPI reporting
- Data modeling with DAX & Power Query

### Call Centre Performance Dashboard
- Agent performance & response time analysis
- Customer service KPI visualization
- Operational insights reporting

### Customer Churn Analysis
- Customer retention trend analysis
- KPI dashboard development
- Business insights for decision making

## 🎯 Career Objective
Aspiring Data & Business Analyst passionate about turning data into actionable insights and supporting data-driven decisions.

## 📫 Connect With Me
- LinkedIn: (Add your link)
- YouTube: LearnWithSpidey (Power BI Tutorials)
